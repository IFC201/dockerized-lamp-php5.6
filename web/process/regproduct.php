<?php
    ...
    $codeProd=consultasSQL::clean_string($_POST['prod-codigo']); // Primer campo (Código de producto)
    ...
    $imgName=$_FILES['img']['name']; // Array{ /'img': campo input de la imagen(name->html) /'name': propiedad(name);}
    $imgType=$_FILES['img']['type']; // " " propiedad(type);
    $imgSize=$_FILES['img']['size']; // " " propiedad(size);
    $imgMaxSize=5120;
            ...
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName)){
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Modelo, Marca, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$priceProd', '$modelProd','$marcaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")){
                            echo '<script>
                                swal({
